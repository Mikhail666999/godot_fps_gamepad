...
# Camera
export(float) var mouse_sensitivity_x = 4.0
export(float) var mouse_sensitivity_y = 3.0
...
func _process(_delta: float) -> void:
	...
	mouse_axis.y = Input.get_action_strength("mous_forward") - Input.get_action_strength("mous_backward")
	mouse_axis.x = Input.get_action_strength("mous_right") - Input.get_action_strength("mous_left")
	camera_rotation()
...
#	mouse_axis = Vector2()
	...
	if mouse_axis.length() > 0:
		var horizontal: float = -mouse_axis.x * (mouse_sensitivity_x)
		var vertical: float = mouse_axis.y * (mouse_sensitivity_y)
		...
		temp_rot.x = clamp(temp_rot.x, -80, 90)
